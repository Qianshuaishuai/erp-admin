...
func (self *TempController) AddPaperTable() {
	...
	cSort, err := self.GetInt("createSort")
	cFlag = true
	if err != nil {
		cFlag = false
	}
	uSort, err := self.GetInt("updateSort")
	uFlag = true
	if err != nil {
		uFlag = false
	}

	result, count := models.GetAddPaperTempList(limit, page, cFlag, uFlag, cSort, uSort)
	list := make([]map[string]interface{}, len(result))
	for k, v := range result {
		row := make(map[string]interface{})
		row["paper_id"] = v.PaperId
		row["paper_name"] = v.Name
		row["paper_course"] = models.GetCourseNameById(v.CourseId)
		row["paper_semester"] = models.GetSemesterNameById(v.SemesterId)
		row["paper_type"] = models.GetTypeNameById(v.TypeId)
		row["paper_create_time"] = beego.Date(v.CreatedAt, "Y-m-d H:i:s")
		row["paper_update_time"] = beego.Date(v.UpdatedAt, "Y-m-d H:i:s")
		row["paper_status"] = ADD_PAPER_STATUS_FLAG[v.Status]
		list[k] = row
	}
